...
namespace SmartShop.Tests
{
    public abstract class SmartShopTestBase : AbpIntegratedTestBase<SmartShopTestModule>
    {
        ...
        protected void UsingDbContext(Action<SmartShopDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<SmartShopDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<SmartShopDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<SmartShopDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SmartShopDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SmartShopDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<SmartShopDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SmartShopDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<SmartShopDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<SmartShopDbContext, Task<T>> func)
        {
            T result;
